		...
		glBegin(g_ViewMode);					// Begin drawing with our selected mode (triangles or lines)
			...
			for(int j = 0; j < pObject->numOfFaces; j++)
			{
				...
				for(int whichVertex = 0; whichVertex < 3; whichVertex++)
				{
					...
					MatStapel.Normale(pObject->pNormals[ index ].x, pObject->pNormals[ index ].y, pObject->pNormals[ index ].z);
					...
					MatStapel.Punkt(pObject->pVerts[ index ].x, pObject->pVerts[ index ].y, pObject->pVerts[ index ].z);
				}
			}
