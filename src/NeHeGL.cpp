...
{
	window->init.width	=	aufloesungen[window->init.resolution][0];
		window->init.height	=	aufloesungen[window->init.resolution][1];
	PostMessage (window->hWnd, WM_TOGGLEFULLSCREEN, 0, 0);				// Send A WM_TOGGLEFULLSCREEN Message
}



BOOL ChangeScreenResolution (GL_WindowInit *window)	// Change The Screen Resolution
{
	DEVMODE dmScreenSettings;											// Device Mode
	ZeroMemory (&dmScreenSettings, sizeof (DEVMODE));					// Make Sure Memory Is Cleared
	dmScreenSettings.dmSize				= sizeof (DEVMODE);				// Size Of The Devmode Structure
	dmScreenSettings.dmPelsWidth		= window->width;						// Select Screen Width
	dmScreenSettings.dmPelsHeight		= window->height;						// Select Screen Height
	dmScreenSettings.dmBitsPerPel		= window->bitsPerPixel;					// Select Bits Per Pixel
...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	if (window->init.isFullScreen == TRUE)								// Fullscreen Requested, Try Changing Video Modes
	{
		...
		if (ChangeScreenResolution (&window->init) == FALSE)
		{
	...
	ReshapeGL (&window->init);				// Reshape Our GL Window

	ZeroMemory (window->Taste, sizeof (CTasten));						// Clear All Keys
...
LRESULT CALLBACK WindowProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	//#######################################################
	//Achtung: Subsystem: Windows  (="/SUBSYSTEM:WINDOWS")
	//			(linker/system)
	//Johannes Gerer
...
void ReshapeGL (GL_WindowInit* window)									// Reshape The Window When It's Moved Or Resized
{
	glViewport (0, 0, (GLsizei)(window->width), (GLsizei)(window->height));				// Reset The Current Viewport
	glMatrixMode (GL_PROJECTION);										// Select The Projection Matrix
	glLoadIdentity ();													// Reset The Projection Matrix
	gluPerspective (window->FOV, (GLfloat)(window->width)/(GLfloat)(window->height),			// Calculate The Aspect Ratio Of The Window
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	window.Taste					= &Taste;								// Window Key Structure
	window.init.application			= &application;							// Window Application
	
	Einstellungen(&window.init); //Johannes Gerer

	ZeroMemory (&Taste, sizeof (CTasten));									// Zero keys Structure

	// Ask The User If They Want To Start In FullScreen Mode?
	if(window.init.FullScreenQuestion)
	{
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
			...
			if (Initialisierung(&window,&Taste) == FALSE)					// Call User Intialization
			{
				// Failure
			...
			Deinitialisierung ();	//Johannes Gerer
